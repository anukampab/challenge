...
class AccountsControllerTest {
	...
	void prepareMockMvc() throws Exception {
		...
		this.mockMvc.perform(post("/v1/accounts").contentType(MediaType.APPLICATION_JSON)
			      .content("{\"accountId\":\"Id-134\",\"balance\":1000}")).andExpect(status().isCreated());
	}
	...
	void createAccountNoBalance() throws Exception {
		this.mockMvc.perform(
				post("/v1/accounts").contentType(MediaType.APPLICATION_JSON).content("{\"accountId\":\"Id-134\"}"))
				.andExpect(status().isBadRequest());
	}
	...
	void createAccountNegativeBalance() throws Exception {
		this.mockMvc.perform(post("/v1/accounts").contentType(MediaType.APPLICATION_JSON)
				.content("{\"accountId\":\"Id-134\",\"balance\":-1000}")).andExpect(status().isBadRequest());
	}
